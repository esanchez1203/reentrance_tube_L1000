...
def place_inner_wlsr_in_argon(
    ...
    if len(ttx_outer_z) > 1:
        ...
        tetratex_inner_pv = g4.PhysicalVolume(
            ...
            "wls_tetratex_inner_argon", lar_cavity_lv, registry=registry
        )
        ...
        if len(tpb_outer_z) > 1:
            ...
            tpb_inner_pv = g4.PhysicalVolume(
                [0, 0, 0], [0, 0, 0, "mm"], wls_inner_lv, 
                "tpb_inner_argon", wls_tetratex_inner_lv, registry=registry
            )
            _add_wls_surfaces(materials, registry, tpb_inner_pv, tetratex_inner_pv, lar_cavity_pv, prefix="inner_")

    print(f"\nInner WLS layer (HIERARCHICAL: Steel -> Gap -> TTX -> TPB -> UAr):")
...
def place_outer_wlsr_in_atmospheric(
    ...
    if len(ttx_outer_z) > 1:
        ...
        tetratex_outer_pv = g4.PhysicalVolume(
            ...
            "wls_tetratex_outer_atmospheric", lar_mother_lv, registry=registry
        )
        ...
        if len(tpb_outer_z) > 1:
            ...
            wls_outer_lv = g4.LogicalVolume(tpb_solid, materials.tpb_on_tetratex, "wls_outer_atmospheric_lv", registry)
            wls_outer_lv.pygeom_color_rgba = [0.0, 1.0, 0.0, 1.0]
            tpb_outer_pv = g4.PhysicalVolume(
                [0, 0, 0], [0, 0, 0, "mm"], wls_outer_lv, 
                "tpb_outer_atmospheric", wls_tetratex_outer_lv, registry=registry
            )
